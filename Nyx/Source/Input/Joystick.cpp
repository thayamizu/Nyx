...
namespace Nyx {
	...
	Joystick::Joystick(HWND hwnd_, int range) 
		: IJoystick(), allowRange(range), joystick(NULL), joystickDevice(NULL), isAcquire(false), isInitialized(false) {
			...
			hr = joystick->EnumDevices(
				DI8DEVCLASS_GAMECTRL, Joystick::EnumJoysticksCallback,(void *)this, DIEDFL_ATTACHEDONLY );
			Assert(hr == DI_OK);
			if( FAILED(hr))
			{
	...
	//-----------------------------------------------------------------------------------------
	//
	int CALLBACK Joystick::EnumJoysticksCallback(const DIDEVICEINSTANCE* lpddi, VOID* lpContext) {
		// 列挙されたジョイスティックへのインターフェイスを取得する。
		HRESULT hr;
		Joystick* joy = reinterpret_cast<Joystick*>(lpContext);
	...
	//-----------------------------------------------------------------------------------------
	// ジョイスティックの軸を列挙する関数
	int  CALLBACK Joystick::EnumAxesCallback(LPCDIDEVICEOBJECTINSTANCE lpddoi, LPVOID pvRef) {
		HRESULT hr;
		...
		Joystick* joy = reinterpret_cast<Joystick*>(pvRef);
