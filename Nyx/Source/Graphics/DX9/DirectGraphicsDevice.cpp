...
namespace Nyx {
	namespace DX9 {
		...
		void DirectGraphicsDevice::ShowCursor(bool showCursor) {
			...
			d3dDevice->ShowCursor(showCursor);
		}
		...
		void DirectGraphicsDevice::GetSamplerState(DWORD sampler,D3DSAMPLERSTATETYPE type, DWORD* value) {
			Assert(d3dDevice);
			d3dDevice->GetSamplerState(sampler, type, value);
		...
		//-----------------------------------------------------------------------------------------
		// ライティングの取得
		void DirectGraphicsDevice::GetLight(D3DLight* light) {
			Assert(d3dDevice);
			d3dDevice->GetLight(0, light);
		}
		...
		void DirectGraphicsDevice::SetLight(Vector3f pos, Vector3f dir, Color3f diffuse, Color3f specular, float range, D3DLIGHTTYPE type) {
			...
			HRESULT hr = d3dDevice->SetLight( 0, &light );
		...
		//-----------------------------------------------------------------------------------------
		// ビューポートの取得
		void DirectGraphicsDevice::GetViewPort(D3DViewPort* vp) {
			Assert(d3dDevice);
		...
		void DirectGraphicsDevice::SetTransform(  D3DTRANSFORMSTATETYPE state, const Matrix44 * matrix)
		{
			...
			d3dDevice->SetTransform(state, reinterpret_cast<const D3DXMATRIX*>(matrix));
		}
		...
		void DirectGraphicsDevice::GetTransform(  D3DTRANSFORMSTATETYPE state, Matrix44 * matrix) {
			Assert(d3dDevice);
			d3dDevice->SetTransform(state, reinterpret_cast<D3DXMATRIX*>(matrix));
		}

		//-----------------------------------------------------------------------------------------
		// デバイスの取得
		Direct3DDevice DirectGraphicsDevice::GetDevice() {
			Assert(d3dDevice);
			return d3dDevice;
		}
		...
		void DirectGraphicsDevice::Clear() {
			...
			d3dDevice->Clear(0, NULL, D3DCLEAR_TARGET |D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB(125, 125, 125, 0), 1.f, 0);
		}
		...
		bool DirectGraphicsDevice::BeginScene() {
			...
			Assert(d3dDevice);
			// 画面を青でクリアする
			d3dDevice->Clear(0, NULL, D3DCLEAR_TARGET |D3DCLEAR_ZBUFFER, D3DCOLOR_XRGB(0, 0, 128), 1.f, 0);
			// 描画開始
