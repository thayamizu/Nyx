...
namespace nyx {
	//-------------------------------------------------------------------------------------------------------
	//
	dsound_streaming_audio_buffer::dsound_streaming_audio_buffer(
		const audio_buffer_desc& bufferDesc, 
		const dsound_ptr dsound, const std::shared_ptr<sound_reader> reader)
		: dsound_stereo_audio_buffer(), waveReader_(reader), bufferDesc_(bufferDesc), offset_(0){
			...
			//通知スレッドの生成
			notifyThreadHandle_ = CreateThread( NULL,0, notify_proc, (void*)this, 0, 0);
	...
	//-------------------------------------------------------------------------------------------------------
	//
	AudioUtility::AUDIO_BUFFER_TYPE dsound_streaming_audio_buffer::get_buffer_type() const {
		return AudioUtility::AUDIO_BUFFER_TYPE_STREAMING;
	}


	//-------------------------------------------------------------------------------------------------------
	//
	void dsound_streaming_audio_buffer::reset() {
		offset_ = 0;
	...
	//-------------------------------------------------------------------------------------------------------
	//
	void dsound_streaming_audio_buffer::write_wave_data(){
	...
	//-------------------------------------------------------------------------------------------------------
	//
	void dsound_streaming_audio_buffer::build_dsound_buffer_desc(DSBUFFERDESC* dsBufferDesc, WAVEFORMATEX& wfx){
		//フラグの設定
	...
	//-------------------------------------------------------------------------------------------------------
	//
	uint64_t _stdcall notify_proc(void* parameter) {
		NYX_ASSERT(parameter != nullptr)
		auto audio = reinterpret_cast<dsound_streaming_audio_buffer*>(parameter);
		audio->notify_thread();
		return 0L;
	}
	...
	//-------------------------------------------------------------------------------------------------------
	//
	void dsound_streaming_audio_buffer::notify_thread() {
		bool isDone = false;
		while( !isDone ) { 
