...
namespace nyx {
	...
	///デフォルトビットレート
	const double DEFAULT_BIT_RATE = 16;

	///デフォルトサンプリングレート
	const double DEFAULT_SAMPLING_RATE = 44100;
	...
	*/
	...
	struct audio_buffer_desc {
		...
		AUDIO_BUFFER_TYPE bufferType;///< オーディオバッファのバッファタイプ
		FOCUS_TYPE		  focusType; ///< オーディオバッファのフォーカスタイプ
	...
	struct audio_effect_desc {
		uint64_t			reserved;///< 予約領域
		AUDIO_EFFECT_TYPE	effectType;///< エフェクトの種類
	};
